...
export const documentEndpoints: ApiEndpoint[] = [
    ...
    },
    {
        ...
        description: '列出已导入的文件/路径（去重；可选文件名过滤；无切块内容）',
        queryParams: [
            {
                name: 'source_file',
                label: '文件名',
                type: 'string',
                placeholder: '留空查全部；或填路径关键字，如 休假 / docs/',
            },
            { name: 'page', label: '页码', type: 'number', default: 1, min: 1 },
            { name: 'page_size', label: '每页条数', type: 'number', default: 10, min: 1, max: 100 },
        ],
        resultView: {
            ...
            serverPaging: true,
            columns: [
                { prop: 'source_file', label: '文件路径', minWidth: 360, showOverflowTooltip: true },
            ],
        },
    },
    {
        ...
        resultView: {
            ...
            columns: [
                ...
                { prop: 'embedding_preview', label: '向量', width: 88, showOverflowTooltip: true },
...
export const corporaEndpoints: ApiEndpoint[] = [
    {
        ...
        description:
            '异步导入到 document_business_chunks：上传 .md/.txt/.zip；资料名必填（可手动填，默认取 ZIP 内文件夹名或文件名）',
        asyncJob: {
        ...
        formParams: [
            {
                ...
                type: 'string',
                required: true,
                placeholder: '如 fastapi；留空则后端按 ZIP 文件夹/文件名推导',
            },
            {
                name: 'category',
                label: '资料分类',
            ...
            { name: 'file', label: '文档/.zip', type: 'file', required: true },
            { name: 'replace_existing', label: '覆盖同名文件', type: 'boolean', default: true },
            {
                name: 'chunk_strategy',
                label: '切分策略',
    ...
    },
    {
        ...
        description: '列出资料库已导入的文件/路径（去重；资料名留空查全部；无切块内容）',
        queryParams: [
            {
                name: 'corpus_name',
                label: '资料名',
                type: 'string',
                placeholder: '留空查全部；或填与导入时一致',
            },
            {
                name: 'source_file',
                label: '文件名',
                type: 'string',
                placeholder: '留空查全部；或填路径关键字',
            },
            { name: 'page', label: '页码', type: 'number', default: 1, min: 1 },
            { name: 'page_size', label: '每页条数', type: 'number', default: 10, min: 1, max: 100 },
        ],
        resultView: {
            ...
            serverPaging: true,
            columns: [
                { prop: 'corpus_name', label: '资料名', width: 140, showOverflowTooltip: true },
                { prop: 'source_file', label: '文件路径', minWidth: 320, showOverflowTooltip: true },
            ],
        },
    },
    {
        ...
        name: '检索过滤建议',
        method: 'GET',
        path: '/documents/corpora/search/suggest-filters',
        description: '根据问题关键词建议分类/资料库等多过滤条件（规则版；可改后再检索）',
        queryParams: [
            { name: 'q', label: '查询文本', type: 'string', required: true, placeholder: '用户问题' },
        ],
    },
    {
        id: 'corpora-search',
        ...
        description: '业务资料库 document_business_chunks 向量/混合检索（分类/多库可过滤；均留空则全库）',
        queryParams: [
            { name: 'categories', label: '分类(多选)', type: 'string', placeholder: '逗号分隔，如 policy,product' },
            { name: 'corpus_names', label: '资料库(多选)', type: 'string', placeholder: '逗号分隔资料名' },
            { name: 'corpus_name', label: '资料名(单)', type: 'string', placeholder: '兼容旧参数；与多选并集' },
            { name: 'q', label: '查询文本', type: 'string', placeholder: '留空返回默认切块列表' },
            { name: 'limit', label: '条数', type: 'number', default: 5, min: 1, max: 50 },
            {
                name: 'min_similarity',
                label: '最低相似度',
    ...
    },
    {
        ...
        description: '业务资料库 document_business_chunks 检索 + 大模型润色（分类/多库可过滤）',
        queryParams: [
            { name: 'categories', label: '分类(多选)', type: 'string', placeholder: '逗号分隔' },
            { name: 'corpus_names', label: '资料库(多选)', type: 'string', placeholder: '逗号分隔资料名' },
            { name: 'corpus_name', label: '资料名(单)', type: 'string', placeholder: '兼容旧参数' },
            { name: 'q', label: '查询文本', type: 'string', placeholder: '留空返回默认切块列表' },
            { name: 'limit', label: '条数', type: 'number', default: 5, min: 1, max: 50 },
            {
                name: 'min_similarity',
