...
def start_game(_: User = Depends(require_permission("89.start"))) -> schemas.ZhaJinhuaStartResponse:
...
def next_round(_: User = Depends(require_permission("89.next-round"))) -> schemas.ZhaJinhuaStartResponse:
    result = get_engine().next_round()
    return schemas.ZhaJinhuaStartResponse(**result)


@router.post("/reset", response_model=schemas.ZhaJinhuaMessageResponse)
def reset_game(_: User = Depends(require_permission("89.reset"))) -> schemas.ZhaJinhuaMessageResponse:
...
async def player_turn(
    player_id: str,
    _: User = Depends(require_permission("89.turn")),
...
async def get_referee_voice(
    _: User = Depends(require_permission("89.referee")),
...
def set_game_access(
    ...
    _: User = Depends(require_permission("89.access")),
...
def get_status(_: User = Depends(require_permission("89.status"))) -> schemas.ZhaJinhuaStatusResponse:
