...
class InsightFeatureBuilder:
    ...
    def _load_aggregates(self) -> dict[str, dict]:
        base_rows = (
            self.db.query(
        ...
        type_rows = (
            ...
            .group_by(FactComplaintSample.user_id, FactComplaintSample.complaint_type)
            .all()
        )
        for user_id, complaint_type, count in type_rows:
        ...
        for user_id, scores in survey_rows:
            if not scores:
                continue
            bucket = agg.setdefault(
                user_id,
